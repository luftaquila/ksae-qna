...
- **탈퇴한 계정의 재로그인은 동의 화면을 다시 거친다.** 방침의 보유 기간이 "탈퇴 시까지"라
  예전 동의는 끝났고, 재가입은 새 가입이다. `auth_callback`은 `deleted_at`이 찍힌 행을 신규처럼
  `pending_signup`으로 보내고, `get_or_create_user()`는 그런 행을 `privacy_consent_version`
  없이는 되살리지 않는다(`ValueError`). 되살릴 때 동의 시각·버전을 지금 값으로 다시 적는다.
  예전에는 콜백이 바로 되살려서 탈퇴 후 재로그인하면 동의 없이 곧바로 채팅으로 들어갔다
